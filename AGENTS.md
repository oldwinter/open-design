...
- 当你维护 flake 时，每个影响 Nix packaging 的 `pnpm-lock.yaml` change 都应刷新 Nix pnpm deps hash。`nix/pnpm-deps.nix` 是 generated lock artifact；使用 `pnpm nix:update-hash`，随后在本地重新运行 `nix flake check --print-build-logs --keep-going`。独立的 `.github/workflows/nix.yml` 会在 Nix/lock inputs 变化时运行 flake check；它**不属于**核心 `ci.yml` / `Validate workspace` / merge queue。Docker image smoke/publish 只存在于 `.github/workflows/docker-image.yml`，同样位于 merge gate 之外。
