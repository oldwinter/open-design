...
- `data/` 是 daemon-managed data root，通过 packaged sidecar launch environment 传给 daemon。
...
Packaged daemon path contract 是显式的：`tools-pack` 写入 namespace/base config，`apps/packaged` 解析 namespace paths，packaged sidecar launcher 通过 launch env 传递 daemon managed paths。Daemon 可以为 non-packaged launches 保留自己的 default fallback，但 packaged runtime 不得依赖从 Electron `userData`、app bundle names 或 ports 推断出的 fallback。
