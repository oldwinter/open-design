...
const CODEX_NOTE_ZH: Readonly<Record<string, string>> = {
  'Agent-agnostic documentation design system': '与 Agent 无关的文档设计系统',
  'Codex covered in the README install steps': 'README 的安装步骤涵盖 Codex',
  'Codex is the README’s preferred path': 'README 首选 Codex 使用路径',
  'Codex named 27× in the README': 'README 中 27 次提到 Codex',
  'Codex named 5× in the README': 'README 中 5 次提到 Codex',
  'Codex named 8× in the README': 'README 中 8 次提到 Codex',
  'Codex named in the repo description': '仓库描述中明确提到 Codex',
  'Explicit Codex path: agent-skills/codex/': '提供明确的 Codex 路径：agent-skills/codex/',
  'No Codex mention; `npx skills add` is agent-agnostic': '未明确提到 Codex；`npx skills add` 与 Agent 无关',
  'No Codex mention; agent-agnostic': '未明确提到 Codex；与 Agent 无关',
  'README: runs from a Codex session': 'README：从 Codex 会话运行',
  '`uipro init --ai codex`': '`uipro init --ai codex`',
  'topic: codex-skills': '主题：codex-skills',
};

const SKILL_BUNDLE_ZH: Readonly<Record<string, string>> = {
  'Bundles 3 commands: find-animation-opportunities / improve-animations / review-animations':
    '包含 3 个命令：find-animation-opportunities / improve-animations / review-animations',
  'Bundles 8 modules: core / timeline / scrolltrigger / plugins / react / frameworks / performance / utils':
    '包含 8 个模块：core / timeline / scrolltrigger / plugins / react / frameworks / performance / utils',
};
...
export const zh: CodexCopyOverride = {
  ...
  cardCta: '查看插件',
  skillCodexNote: (note) => CODEX_NOTE_ZH[note] ?? note,
  skillBundle: (bundle) => SKILL_BUNDLE_ZH[bundle] ?? bundle,
  detailWhatIsIt: '这是什么',
