...
describe('buildRedirectLoopBlockedDoc locale', () => {
  it('renders Simplified Chinese with the matching language tag when the host locale is zh-CN', () => {
    // Given
    ...
    expect(doc).toContain('预览已停止：检测到重定向循环');
    expect(doc).not.toContain('Preview stopped: redirect loop detected');
  });

  it('keeps English copy and language metadata for the English locale', () => {
    // Given / When
    const doc = buildRedirectLoopBlockedDoc('en');

    // Then
    expect(doc).toContain('<html lang="en">');
    expect(doc).toContain('Preview stopped: redirect loop detected');
    expect(doc).not.toContain('预览已停止');
  });

  it('renders Traditional Chinese with the matching language tag', () => {
    // Given / When
    const doc = buildRedirectLoopBlockedDoc('zh-TW');

    // Then
    expect(doc).toContain('<html lang="zh-TW">');
    expect(doc).toContain('預覽已停止：偵測到重新導向循環');
    expect(doc).not.toContain('预览已停止：检测到重定向循环');
  });
});
