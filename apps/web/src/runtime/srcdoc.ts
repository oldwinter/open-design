 ...
 */
export function buildRedirectLoopBlockedDoc(locale?: string): string {
  const hostLocale = locale ?? (
    typeof document === 'undefined' ? 'en' : document.documentElement.lang
  );
  const normalizedLocale = hostLocale.toLowerCase();
  const copy = normalizedLocale === 'zh-tw' || normalizedLocale.startsWith('zh-hant')
    ? {
        lang: 'zh-TW',
        title: '預覽已停止：偵測到重新導向循環',
        body: '此文件持續重新導向至自身，可能導致預覽卡住。請重新載入預覽後再試。',
      }
    : normalizedLocale === 'zh-cn' || normalizedLocale.startsWith('zh-hans')
      ? {
          lang: 'zh-CN',
          title: '预览已停止：检测到重定向循环',
          body: '此文档不断重定向到自身，可能导致预览卡死。请重新加载预览后重试。',
        }
      : {
          lang: 'en',
          title: 'Preview stopped: redirect loop detected',
          body: 'This document kept redirecting to itself, which would freeze the preview. Reload the preview to try again.',
        };
  return `<!doctype html>
<html lang="${copy.lang}">
  <head>
