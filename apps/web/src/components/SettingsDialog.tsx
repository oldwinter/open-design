...
export function localizeSiliconFlowProviderLabel(
  label: string,
  locale: Locale,
): string {
  if (locale === 'zh-CN') {
    if (label === 'SiliconFlow (CN)') return 'SiliconFlow（中国区）';
    if (label === 'SiliconFlow (Global)') return 'SiliconFlow（全球）';
  }
  if (locale === 'zh-TW') {
    if (label === 'SiliconFlow (CN)') return 'SiliconFlow（中國區）';
    if (label === 'SiliconFlow (Global)') return 'SiliconFlow（全球）';
  }
  return label;
}
...
export function SettingsDialog({
    ...
    try {
      ...
      setAgentTestState({
        ...
        result: {
          ...
          detail: t('settings.byokSecureStorageUnavailable'),
        },
      });
      trackSettingsCliTestResult(analytics.track, {
        page_name: 'settings',
  ...
  const byokProviderPresets: ReadonlyArray<ByokProviderPreset> = [
    ...BYOK_PROVIDER_PRESETS.map((provider) => ({
      ...provider,
      title: localizeSiliconFlowProviderLabel(provider.title, locale),
    })),
    {
      id: 'custom',
  ...
  const localizedProtocolProviders = useMemo(
    () =>
      protocolProviders.map((provider) => ({
        ...provider,
        label: localizeSiliconFlowProviderLabel(provider.label, locale),
      })),
    [locale, protocolProviders],
  );
            ...
            <section className="settings-section settings-section-card settings-section-byok">
              ...
              {showProviderPreset ? (
                <ByokProviderPicker
                  ...
                  providers={localizedProtocolProviders}
