...
安全：
  API key 只能通过 --api-key-stdin 从 stdin 读取。切勿把 key 放入命令参数、
  chat、环境变量、manifest 或文件。只有无需凭据的本地或自托管 endpoint
  才能使用 --no-api-key。
`;
...
function printProfile(profile: ByokCredentialProfile): string {
  const credential = profile.configured
    ? `已配置${profile.keyTail ? ` (…${profile.keyTail})` : ''}`
    : '未配置';
  ...
  try {
    ...
    if (parsed.command === 'test') {
      ...
      stdout(useJson
        ...
        : `${result.ok ? '连接成功' : '连接失败'}（${result.kind}，${result.latencyMs} ms）${result.model ? `，使用 ${result.model}` : ''}。\n`);
