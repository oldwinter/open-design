...
describe('secure BYOK CLI', () => {
  it('keeps commands machine-stable while localizing human-readable help', async () => {
    const fetchMock = vi.fn<typeof fetch>();
    const { deps, stdout } = dependencies(fetchMock);

    const result = await runByokToolCli(['help'], deps);

    expect(result.exitCode).toBe(0);
    expect(stdout.join('')).toContain('od byok save');
    expect(stdout.join('')).toContain('安全');
