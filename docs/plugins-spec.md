...
| 3 | Daemon-managed plugin location | plugin bundle | 本 spec 不得定义 daemon data paths；记录 storage 前，先阅读根目录 `AGENTS.md` → **Daemon data directory contract** |
| 4 | User-global skill location | legacy `SKILL.md` | 本 spec 不得定义 daemon data paths；记录 storage 前，先阅读根目录 `AGENTS.md` → **Daemon data directory contract** |
...
- OD desktop UI 安装相同 daemon 与相同 CLI；它只是加了一个窗口。用户之后安装 desktop 时，会看到 headless 流程创建的同一批 projects、plugins、history。不存在「headless project format」与「desktop project format」之分。本 spec 不得定义 daemon data paths；修改或记录 shared storage 前，先阅读根目录 `AGENTS.md` → **Daemon data directory contract**。
...
daemon 当前已经拥有 project filesystems（或者 imported folder 的 `metadata.baseDir`）。这些命令以 project 为 scope：agent 不需要知道 project 在磁盘哪里。本 spec 不得定义 daemon data paths；修改或记录 project storage 前，先阅读根目录 `AGENTS.md` → **Daemon data directory contract**。
...
- 同一个 project 之后在 OD desktop UI 中打开时，会显示 headless run 产生的完整 conversation history、files 和 artifacts。本 spec 不得定义 daemon data paths；修改或记录 shared storage 前，先阅读根目录 `AGENTS.md` → **Daemon data directory contract**。
...
| Local services / db | Cursor 的 background indexing / state | OD daemon-managed state。Storage paths 只由根目录 `AGENTS.md` → **Daemon data directory contract** 管辖。 |
...
本 deployment draft 不得定义 daemon data paths、mount paths 或 persistence examples。选择、记录或修改 persistence 前，必须先阅读根目录 [`AGENTS.md`](../AGENTS.md) → **Daemon data directory contract**。该 section 是强制要求，也是唯一 truth source。
...
# 只有在阅读 root AGENTS.md -> Daemon data directory contract 后，才能设置 daemon storage env vars。
...
任何能通过 desktop UI 设置的内容，也都能通过 `docker exec od od config set ...` 设置。本文档不得提供具体 storage paths。
...
这里有意省略 persistent server storage examples。编写这类示例前，必须先阅读根目录 [`AGENTS.md`](../AGENTS.md) → **Daemon data directory contract**。
...
1. **OD 把 design substrate stage 进 project cwd。** 按 §14.3，daemon 把 SKILL.md / DESIGN.md / craft 写入 staged skill-context directory，并通过 `od files` 把生成 artifact 写入 project cwd。cwd 通过 `od project info <id> --json | jq -r .cwd` 可发现。
