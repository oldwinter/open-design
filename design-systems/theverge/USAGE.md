...
- Near-black editorial canvas（`#131313`）作为 default surface — homepage 不使用 light mode
- Acid-mint `#3cffd0` + ultraviolet `#5200ff` 作为 hazard-tape accents，绝不是安静的 background wash
- 巨大的 Manuka display headlines 最高到 107px — mainstream tech media 中最响亮的 type move
- 一切都是 rounded pill-card：20/24/30/40px corner radii，绝不方正
