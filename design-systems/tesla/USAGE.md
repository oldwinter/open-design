...
- Full-viewport hero sections（100vh）由 cinematic car photography 主导，只搭配 minimal overlay UI
- Near-zero UI decoration：页面任何位置都不使用 shadows、gradients、borders 或 patterns
- 单一 accent color — Electric Blue（`#3E6AE1`）— 只用于 primary CTA buttons
- Universal Sans font family（Display + Text）统一 web、app 和 in-car interfaces
