...
- 四层 green brand system（Starbucks / Accent / House / Uplift），每层都映射到不同 surface role — 不是单一的“brand green”
- Gold 只保留给 Rewards-status moments；不要作为 general-purpose accent
- 使用 warm-neutral canvas（`#f2f0eb` / `#edebe9`）而不是冷白色 — 呼应 café materials
- 自定义 proprietary typeface（SoDoSans）搭配紧凑的 `-0.16px` letter-spacing，构成统一 voice
