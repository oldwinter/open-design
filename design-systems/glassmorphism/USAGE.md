...
- 视觉风格： clean, high-contrast, bold, enterprise, liquidglass effect, glassmorphism
- 色彩取向： primary, neutral, success, warning, danger, info, surface/subtle layers
- Design intent：保持 output 可识别为这一 style family，同时保留 usability 和 readability。
- Primary：`#1856FF` — 来自 style foundations 的 token。
