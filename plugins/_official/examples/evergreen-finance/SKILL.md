...
description: "高端 Kova 金融科技落地页：全屏正反向 boomerang 视频背景的 Hero、FadeUp 模糊上浮动画、悬浮仪表盘卡片（储蓄折线 + 支出柱状图）、左右分栏的方形自动播放视频证言区，以及含图片卡与甜甜圈支出图的四宫格功能区。"
version: 0.1.0
od:
  mode: prototype
  surface: web
...
# Evergreen Finance — Kova 金融科技落地页

> 中文说明：高端 Kova 金融科技落地页：全屏正反向 boomerang 视频背景的 Hero、FadeUp 模糊上浮动画、悬浮仪表盘卡片（储蓄折线 + 支出柱状图）、左右分栏的方形自动播放视频证言区，以及含图片卡与甜甜圈支出图的四宫格功能区。
>
