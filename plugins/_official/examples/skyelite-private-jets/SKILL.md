...
description: "高端私人飞机落地页 Hero：全屏自动播放的 CloudFront 视频背景、max-w-7xl 导航栏配 Lucide 汉堡菜单移动端下拉，以及居中重叠的双行标题（Premium. / Accessible.）和 Discover + Book Now 胶囊按钮。"
version: 0.1.0
od:
  mode: prototype
  surface: web
...
# SkyElite 私人飞机 — 高端落地页 Hero

> 中文说明：高端私人飞机落地页 Hero：全屏自动播放的 CloudFront 视频背景、max-w-7xl 导航栏配 Lucide 汉堡菜单移动端下拉，以及居中重叠的双行标题（Premium. / Accessible.）和 Discover + Book Now 胶囊按钮。
>
> 参考实现位于 `example.html`。请从它开始，保留已锁定的视觉 token、布局、动画、媒体 URL 和响应式行为；只按用户需求调整文案、数据和必要素材。下面的英文规格保留为精确执行参数。
