...
description: "暗色高端 3D 创作者作品集落地页：全屏 Hero（渐变标题 + 鼠标磁吸 3D 人像）、随滚动横移的图片 marquee、带四角 3D 装饰与逐字揭示文案的 About、白底 Services 列表，以及滚动缩放堆叠的项目卡片。"
version: 0.1.0
od:
  mode: prototype
  surface: web
...
> 中文说明：暗色高端 3D 创作者作品集落地页：全屏 Hero（渐变标题 + 鼠标磁吸 3D 人像）、随滚动横移的图片 marquee、带四角 3D 装饰与逐字揭示文案的 About、白底 Services 列表，以及滚动缩放堆叠的项目卡片。
>
